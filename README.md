...
[![Backend Tests](https://img.shields.io/badge/backend_tests-27%2F27_passed-brightgreen.svg)]()
[![Frontend Tests](https://img.shields.io/badge/frontend_tests-3%2F3_passed-brightgreen.svg)]()
...
### Run All Backend Tests (27 Tests)
```bash
cd backend
npm test
```
Tests executed:
- `tests/domain/readinessService.test.js`: Domain boundary thresholds, tie-breaking, missing competencies, mathematical invariants.
- `tests/integration/api.test.js`: Authentication, authorization, tenant isolation, non-disclosing 404, optimistic concurrency.
- `tests/idempotency/idempotency.test.js`: 3 parallel identical requests producing 1 attempt, stored replay, fingerprint mismatch rejection.
- `tests/integration/failureInjection.test.js`: Simulated MongoDB outage, outbox persistence, and recovery flushing without duplicate events.

### Run Frontend Resilience Tests (3 Tests)
```bash
cd frontend
npm test
```
Tests executed:
- `frontendResilience.test.tsx`: Out-of-order response discard, `AbortController` cancellation on fast tenant switch, and 409 Conflict UI rendering.
...
- [x] All 27 backend tests and 3 frontend tests pass with zero failures
